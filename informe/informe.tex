...
% COMPLETAR: nombre del estudiante
...
\begin{abstract}
En este laboratorio se comparan algoritmos que tienen la misma complejidad
pero distinto patrón de acceso a memoria: los dos pares de bucles anidados
del libro de Pacheco, la multiplicación clásica de matrices y la
multiplicación por bloques. Los programas se ejecutaron en dos equipos con
distinto tamaño de caché y se analizaron con Valgrind (Cachegrind). Los
resultados muestran que recorrer una matriz por columnas puede ser hasta
20 veces más lento que por filas, y que la multiplicación por bloques puede
ser hasta 10,9 veces más rápida que la clásica, aunque en todos los casos
el número de operaciones es el mismo. La diferencia se explica por los
fallos de caché.
\end{abstract}
...
El objetivo de este trabajo es comprobar esto de forma experimental,
midiendo tiempos y usando Cachegrind para contar los fallos de caché.
...
En los dos casos el bucle externo da $MAX$ vueltas y el interno otras $MAX$,
así que el cuerpo se ejecuta
\[
\sum_{i=0}^{MAX-1} \sum_{j=0}^{MAX-1} 1 = MAX^2
\]
veces. Como el cuerpo hace un número fijo de operaciones, los dos pares son
$O(MAX^2)$. Solo cambia el orden en que se recorre la matriz.
...
por columnas es más lento, y la diferencia crece con $MAX$: en la laptop
...
$MAX$ posiciones, así que casi cada acceso cae en una línea distinta y se
produce un fallo de caché.

También se ve la influencia del tamaño de la caché L3. En la laptop (L3 de
3 MB) la diferencia salta de 2 a 11,6 veces entre $MAX=500$ y $MAX=1000$,
que es justo cuando $A$ pasa de 1,9 MB a 7,6 MB y deja de caber en L3. En
la PC (L3 de 12 MB) con $MAX=1000$ la matriz todavía cabe y la diferencia es
solo de 2,4 veces; el salto grande aparece en $MAX=2000$.
...
Para verificar la implementación, se comparó su resultado elemento por
elemento con el de la multiplicación clásica, usando matrices con valores
distintos en cada posición, en 56 combinaciones de tamaño ($n$ de 1 a 600,
incluyendo valores que no son múltiplos del bloque) y de bloque ($b$ de 1 a
1000). En todos los casos los resultados fueron idénticos.
...
muestra los tamaños más interesantes; ``Mejora'' indica cuántas veces es
más rápida que la clásica.
...
\caption{Efecto del tamaño de bloque para $n=1000$. La línea roja es la multiplicación clásica.}
\label{fig:tambloque}
\end{figure}
...
se pierde algo por el costo de controlar los seis bucles.
...
\[
t \approx (\text{operaciones}) \times t_{op} + (\text{fallos de caché}) \times t_{fallo}.
\]
...
porque las 8 vías del conjunto también las usan las líneas de $A$ y $C$.
...
\item \textbf{Instrucciones ejecutadas:} la versión por bloques ejecuta
      ...
      porque tiene tres bucles extra y las llamadas a \texttt{min}. Aun así
...
\item \textbf{Fallos en la caché de datos:} aquí está la diferencia. En la
      ...
      bloques con $n=500$ falla solo 1 de cada 200.
...
Seleccionando el evento \texttt{D1mr} (fallos de lectura en D1) y la vista
de código fuente se puede ver qué línea produce los fallos. Lo mismo se
obtiene en texto con \texttt{cg\_annotate}. En la versión clásica con
$n=512$, la línea
...
\noindent tiene 134\,547\,457 fallos D1, es decir el \textbf{99,8\,\%} de
todos los fallos del programa. En la versión por bloques con $n=500$, la
misma línea solo tiene 1\,176\,499.

\begin{figure}[H]
\centering
\IfFileExists{../graficas/kcachegrind.png}{%
  \includegraphics[width=0.9\textwidth]{../graficas/kcachegrind.png}%
}{%
  \fbox{\parbox{0.8\textwidth}{\centering\vspace{1.5cm}
  COMPLETAR: insertar captura de KCachegrind\\
  (guardarla como \texttt{graficas/kcachegrind.png})\vspace{1.5cm}}}%
}
\caption{Vista de KCachegrind para la multiplicación clásica ($n=512$), evento D1mr.}
...
\begin{enumerate}
\item Los dos pares de bucles de Pacheco hacen las mismas $MAX^2$ iteraciones,
